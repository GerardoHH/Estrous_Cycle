...
class LossAccPlotter(object):
    ...
    def add_values(self, x_index, loss_train=None, loss_val=None, acc_train=None,
                   acc_val=None, redraw=True, draw_regression = True ):
    ...
    def redraw(self, draw_regression = True):
        ...
        if self.show_averages:
            ...
            ncol += 1
        if self.show_regressions and draw_regression :
    ...
    def _dict_to_list(self, dict):
        ...
        lista = list(self.values_loss_train.items())
        
        for i in range(len(self.values_loss_train.items())):
                list_keys.append  ( lista[i][0] )
                list_values.append( lista[i][1] )
                
        return list_keys, list_values
    
    def _dict_to_list_values(self, dict):
    ...
    def _redraw_main_lines(self):
        ...
        if ax1:
            ...
            keys, values = self._dict_to_list(self.values_loss_train)
            h_lt, = ax1.plot(keys, values,
                             ls_loss_train, label="loss train", alpha=alpha_main)
            #h_lt, = ax1.plot(self.values_loss_train.keys(), self.values_loss_train.values(),
            #                 ls_loss_train, label="loss train", alpha=alpha_main)
            
            keys, values = self._dict_to_list(self.values_loss_val)
            h_lv, = ax1.plot(keys, values,
        ...
        if ax2:
            keys, values = self._dict_to_list(self.values_acc_train)
            h_at, = ax2.plot(keys, values,
                             ls_acc_train, label="acc. train", alpha=alpha_main)
            #h_at, = ax2.plot(self.values_acc_train.keys(), self.values_acc_train.values(),
            #                 ls_acc_train, label="acc. train", alpha=alpha_main)
            keys, values = self._dict_to_list(self.values_acc_val.values())
            h_av, = ax2.plot(keys, values,
    ...
    def _redraw_averages(self):
        ...
        if ax1:
            ...
            keys = self._dict_to_list_values(lt_sma_x)
            
            h_lt, = ax1.plot(keys, lt_sma_y, self.linestyles["loss_train_sma"],
                             label="train loss (avg %d)" % (self.averages_period,),
                             alpha=alpha_sma)
            #h_lt, = ax1.plot(lt_sma_x, lt_sma_y, self.linestyles["loss_train_sma"],
            
            keys = self._dict_to_list_values(lv_sma_x)
            h_lv, = ax1.plot(keys, lv_sma_y, self.linestyles["loss_val_sma"],
        ...
        if ax2:
            # for accuracy chart
            keys = self._dict_to_list_values(at_sma_x)
            #h_at, = ax2.plot(at_sma_x, at_sma_y, self.linestyles["acc_train_sma"],
            h_at, = ax2.plot(keys, at_sma_y, self.linestyles["acc_train_sma"],
                             label="train acc (avg %d)" % (self.averages_period,),
                             alpha=alpha_sma)
            keys = self._dict_to_list_values(av_sma_x)
            h_av, = ax2.plot(keys, av_sma_y, self.linestyles["acc_val_sma"],
    ...
    def _calc_regression(self, x_values, y_values):
        ...
        values = self._dict_to_list_values(x_values )
        last_x = values[-1]
        #last_x = x_values[-1]
        ...
        x_values_a = self._dict_to_list_values( x_values )
        y_values_a = self._dict_to_list_values( y_values )
        fit = np.polyfit(x_values_a[-n_backward:], y_values_a[-n_backward:],
                         self.poly_degree)
        poly = np.poly1d(fit)
